...
// EXTERNAL MODULE: blockbook-fetcher (ignored)
var blockbook_fetcher_ignored_ = __webpack_require__(436);
...
const blockbook = globalThis?.blockbook || blockbook_fetcher_ignored_;
...
class CoinProvider {
  ...
  async estimateFee() {
    ...
    return Math.floor(parseCoins(Number(result) / 1e3));
  }
...
class CoinWallet {
  ...
  constructor(privateKey, provider, options) {
    ...
    const keyPair = privateKey ? ECPair.fromWIF(privateKey, this.network) : generateRandom ? ECPair.makeRandom({ network: this.network }) : null;
    ...
    this.privateKey = keyPair ? keyPair.toWIF() : void 0;
    this.privateKeyWithPrefix = keyPair ? `${electrumKeys[this.addrType]}:${this.privateKey}` : void 0;
...
class MnemonicWallet extends CoinWallet {
  ...
  constructor(mnemonic, provider, options) {
    super(void 0, provider, {
      ...options,
      generateRandom: false
    });
    const generateRandom = options?.generateRandom ?? true;
    this.mnemonic = mnemonic || (generateRandom ? wallet_bip39.generateMnemonic(128) : "");
